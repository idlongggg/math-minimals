...
import { QUICK_EXAMPLES, TABLE_DIVISORS, TABLE_ROWS_COUNT, TABLE_START_NUMBER } from '../constants';
...
function DivisionTable() {
  ...
  return (
    <Card>
      <CardHeader title={`Bảng chia có dư mẫu (chia cho ${TABLE_DIVISORS.join(', ')})`} />
      <CardContent>
        <Box sx={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {Array.from({ length: TABLE_ROWS_COUNT }, (_, i) => i + TABLE_START_NUMBER).map(
                (num) => {
